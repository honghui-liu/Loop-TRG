...
def init_S(ts_T, d_cut):
    ...
    # transpose ts_TB
    ts_T_list.append(np.einsum('lijk->ijkl',ts_T[1]))

    ts_Result = []
    for i in range(2):
        # convert T[0]/T[1] to matrix mat[0], mat[1]
        dim = ts_T_list[i].shape
        dim1 = dim[0]*dim[1]
        dim2 = dim[2]*dim[3]
        mat = ts_T_list[i].reshape((dim1, dim2))

        # set cutoff criterion
        dc = min(d_cut, dim1, dim2)
        
        # do svd for mat
        mat_U, s, mat_V = np.linalg.svd(mat, full_matrices=True)
        ...
        ts_S1 = mat1.reshape((dim[0],dim[1],dc))
        ts_S2 = mat2.reshape((dc,dim[2],dim[3]))
...
def loop_optimize(ts_T, d_cut, error_limit, round_limit):
    error = np.inf
    # initialize the 8 tensors S
    ts_S_old = init_S(ts_T, d_cut)
    ...
    back_or_forth = 0
    loop_round = 0
    while ((error > error_limit) or (loop_round < round_limit)):
        cost_old = cost_func(0, ts_S_new, ts_T)
        if back_or_forth % 2 == 0:
            for i in np.arange(0, num, 1, dtype=int):       # from 0 to 7
            ...
            for i in np.arange(num-1, -1, -1, dtype=int):    # from 7 to 0
                ts_N = tensor_N(i, ts_S_new)
                ts_W = tensor_W(i, ts_S_new, ts_T)
                ts_S_new[i] = optimize_S(ts_N, ts_W)
            back_or_forth -= 1
