...
#  main.py
#  Loop_TRG
#  Main program for Loop-TRG calculation
#
...
filename = 'temperature' + ('%.3f' % temperature) + '.txt'
with open(filename, 'w+') as f:

    for i in range(16):
        # entanglement filtering
        ts_TN_A0, ts_TN_B0 = flt.filter(ts_TN_A0, ts_TN_B0, 1.0E-12)
        # loop optimize to find the new tensors
        # ts_TN_A0 -> fA * ts_TN_A1 = ts_T_A1
        # ts_TN_B0 -> fB * ts_TN_B1 = ts_T_B1
        ts_T_A1, ts_T_B1, loop_error = opt.loop_optimize((ts_TN_A0,ts_TN_B0), 16, 1.0E-5, 500)
